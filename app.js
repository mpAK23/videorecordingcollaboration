...
async function exportMix() {
    ...
    function drawFrame() {
        ...
        if (count === 1) {
            ctx.drawImage(tracksToExport[0].vidEl, 0, 0, w, h);
        } else if (count === 2) {
            ctx.drawImage(tracksToExport[0].vidEl, 0, 0, w/2, h);
            ctx.drawImage(tracksToExport[1].vidEl, w/2, 0, w/2, h);
        } else if (count === 3) {
            ctx.drawImage(tracksToExport[0].vidEl, 0, 0, w/3, h);
            ctx.drawImage(tracksToExport[1].vidEl, w/3, 0, w/3, h);
            ctx.drawImage(tracksToExport[2].vidEl, 2*w/3, 0, w/3, h);
        } else if (count === 4) {
            ctx.drawImage(tracksToExport[0].vidEl, 0, 0, w/2, h/2);
            ctx.drawImage(tracksToExport[1].vidEl, w/2, 0, w/2, h/2);
            ctx.drawImage(tracksToExport[2].vidEl, 0, h/2, w/2, h/2);
            ctx.drawImage(tracksToExport[3].vidEl, w/2, h/2, w/2, h/2); 
        }
